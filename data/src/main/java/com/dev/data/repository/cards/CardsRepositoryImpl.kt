...
package com.dev.data.repository.cards
...
import com.dev.data.core.log.MyLogger.Debug.logDebug
import com.dev.data.core.log.MyLogger.Debug.logError
import com.dev.data.core.log.MyLogger.Debug.logWtf
	...
	private fun cardsQuery(user: UserItem): List<Query> = with(user.location) {
		...
		bounds.map { bound ->
			...
			fs.collection(USERS_COLLECTION)
				...
				.whereIn(
					USERS_FILTER_GENDER,
					if (user.preferences.gender != SelectionPreferences.PreferredGender.EVERYONE) listOf(user.preferences.gender.name)
